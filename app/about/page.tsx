export default function About() {
  return (
    <div className="max-w-6xl mx-auto px-4">
      
      {/* Header */}
      <div className="text-center mb-20">
        <div className="inline-flex items-center space-x-4 p-6 bg-card/80 backdrop-blur-xl rounded-3xl border border-muted mb-12 shadow-2xl">
          <div className="w-6 h-6 bg-red-500 rounded-full" />
          <div className="w-6 h-6 bg-yellow-500 rounded-full" />
          <div className="w-6 h-6 bg-green-500 rounded-full" />
          <div className="ml-4 font-mono text-sm text-muted-foreground">
            ~/about
          </div>
        </div>

        <h1 className="text-3xl sm:text-5xl md:text-7xl lg:text-8xl font-mono tracking-widest bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent mb-4">
          ABOUT/
        </h1>

        <div className="text-lg text-muted-foreground font-mono">
          profil, formation
        </div>
      </div>

      {/* Grid */}
      <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-start mb-20">

        {/* Profil */}
        <div className="group min-h-[500px] hover:shadow-2xl relative">
          <div className="bg-card border-2 border-slate-800/50 rounded-3xl p-6 md:p-8 h-full flex flex-col shadow-xl hover:border-slate-700/70 hover:shadow-black/20 glass overflow-hidden">

            {/* Terminal header */}
            <div className="flex items-center space-x-3 mb-8 p-4 bg-slate-900/30 rounded-2xl border border-slate-800/50">
              <div className="w-3 h-3 bg-red-500 rounded-full" />
              <div className="w-3 h-3 bg-yellow-500 rounded-full" />
              <div className="w-3 h-3 bg-green-500 rounded-full" />
              <div className="ml-3 font-mono text-xs text-slate-400 tracking-wider">
                mariam.chairi
              </div>
            </div>

            {/* Avatar */}
            <div className="text-center mb-10">
              <div className="w-20 h-20 bg-gradient-to-br from-slate-800/50 to-black/30 rounded-full mx-auto mb-6 flex items-center justify-center border-2 border-slate-700/70 shadow-2xl">
                <div className="w-16 h-16 bg-gradient-to-br from-slate-600 to-slate-400 rounded-full shadow-xl" />
              </div>

              <h2 className="font-mono text-2xl md:text-3xl font-bold text-slate-100 mb-2 tracking-tight">
                Mariam Chairi
              </h2>

              <p className="text-base md:text-lg text-slate-400 font-mono uppercase tracking-wider">
                Software Engineer
              </p>
            </div>

            {/* Infos */}
            <div className="space-y-6 flex-1 min-w-0">

              {/* Location */}
              <div className="p-6 bg-slate-900/40 border border-slate-800/60 rounded-2xl">
                <h3 className="font-mono text-lg font-semibold text-slate-300 mb-4 flex items-center">
                  <svg
                    className="w-5 h-5 mr-4 text-slate-400"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
                      fillRule="evenodd"
                      d="M5.05 4.05a7 7 0 119.9 9.9L10 18.9l-4.95-4.95a7 7 0 010-9.9zM10 11a2 2 0 100-4 2 2 0 000 4z"
                      clipRule="evenodd"
                    />
                  </svg>

                  <span>Location</span>
                </h3>

                <p className="text-slate-400 font-mono text-sm break-words">
                  Tétouan, Maroc
                </p>
              </div>

              {/* Languages */}
              <div className="p-6 bg-slate-900/40 border border-slate-800/60 rounded-2xl flex-1">
                <h3 className="font-mono text-lg font-semibold text-slate-300 mb-4 flex items-center">
                  📚 <span className="ml-3">Languages</span>
                </h3>

                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">

                  <div className="bg-slate-800/50 px-4 py-3 rounded-xl text-sm text-slate-300 border border-slate-700/70 font-mono text-center break-words">
                    🇲🇦 Arabe <strong>Native</strong>
                  </div>

                  <div className="bg-slate-800/50 px-4 py-3 rounded-xl text-sm text-slate-300 border border-slate-700/70 font-mono text-center break-words">
                    🇫🇷 Français <strong>Avancé</strong>
                  </div>

                  <div className="bg-slate-800/50 px-4 py-3 rounded-xl text-sm text-slate-300 border border-slate-700/70 font-mono text-center break-words">
                    🇺🇸 Anglais <strong>Interméd</strong>
                  </div>

                </div>
              </div>
            </div>
          </div>
        </div>

        {/* Formation */}
        <div className="group min-h-[500px] hover:shadow-2xl relative">
          <div className="bg-card border-2 border-slate-800/50 rounded-3xl p-6 md:p-8 h-full flex flex-col shadow-xl hover:border-slate-700/70 hover:shadow-black/20 glass overflow-hidden">

            {/* Title */}
            <div className="text-center mb-12">
              <h3 className="font-mono text-2xl md:text-3xl font-bold text-slate-100 mb-2">
                Formation
              </h3>

              <div className="h-1 w-20 bg-primary mx-auto rounded-full" />
            </div>

            {/* Diplômes */}
            <div className="flex-1 space-y-4 min-w-0">

              {/* Master */}
              <div className="transition-all duration-300 p-6 border border-slate-800/40 rounded-2xl hover:border-slate-700/60">
                <div className="flex items-start space-x-4">

                  <div className="w-3 h-3 bg-primary rounded-full mt-2 flex-shrink-0" />

                  <div className="flex-1 min-w-0">
                    <h4 className="font-mono font-semibold text-lg text-slate-200 mb-1 break-words">
                      Master Ingénierie Informatique
                    </h4>

                    <p className="text-slate-400 font-mono text-sm break-words">
                      Université Abdelmalek Essaâdi
                    </p>

                    <p className="text-xs text-slate-500 font-mono mt-1 break-words">
                      Faculté des Sciences Tétouan, Maroc
                    </p>
                  </div>
                </div>
              </div>

              {/* Licence */}
              <div className="transition-all duration-300 p-6 border border-slate-800/40 rounded-2xl hover:border-slate-700/60">
                <div className="flex items-start space-x-4">

                  <div className="w-3 h-3 bg-primary rounded-full mt-2 flex-shrink-0" />

                  <div className="flex-1 min-w-0">
                    <h4 className="font-mono font-semibold text-lg text-slate-200 mb-1 break-words">
                      Licence Mathématique et Informatique
                    </h4>

                    <p className="text-slate-400 font-mono text-sm break-words">
                      Université Abdelmalek Essaâdi
                    </p>

                    <p className="text-xs text-slate-500 font-mono mt-1 break-words">
                      Faculté des Sciences Tétouan, Maroc
                    </p>
                  </div>
                </div>
              </div>

              {/* Bac */}
              <div className="transition-all duration-300 p-6 border border-slate-800/40 rounded-2xl hover:border-slate-700/60">
                <div className="flex items-start space-x-4">

                  <div className="w-3 h-3 bg-primary rounded-full mt-2 flex-shrink-0" />

                  <div className="flex-1 min-w-0">
                    <h4 className="font-mono font-semibold text-lg text-slate-200 mb-1 break-words">
                      Baccalauréat Sciences Physiques
                    </h4>

                    <p className="text-slate-400 font-mono text-sm break-words">
                      Lycée Kadiayad de Tétouan
                    </p>

                    <p className="text-xs text-slate-500 font-mono mt-1 break-words">
                      Tétouan, Maroc
                    </p>
                  </div>
                </div>
              </div>

            </div>

          </div>
        </div>

      </div>
    </div>
  )
}